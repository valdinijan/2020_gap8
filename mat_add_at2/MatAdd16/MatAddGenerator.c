...
void LoadMatAddLibrary()
{
    ...
    LibKernel("MatSumPar", CALL_PARALLEL,
        CArgs(5,
            TCArg("unsigned short * __restrict__", "In1"),
            TCArg("unsigned short * __restrict__", "In2"),
...
void GenerateMatAddPar(char *UserKernelName, int W, int H)
{
    ...
    UserKernel(UserKernelName,
        ...
        KerArgs(3,
            ...
            KerArg("In1", KerArgSpace(1,KER_ITER_TILE0), O_IN|O_DB, W, H,  sizeof(unsigned short), 0, 0, 0, "In1"),
            // Second kernel argument, bound to In2. In2 in L2, double buffered access
            KerArg("In2", KerArgSpace(1,KER_ITER_TILE0), O_IN|O_DB, W, H,  sizeof(unsigned short), 0, 0, 0, "In2"),
            // Third kernel argument, bound to Out. Out in L2, double buffered access
            KerArg("Out", KerArgSpace(1,KER_ITER_TILE0), O_OUT|O_DB, W, H, sizeof(unsigned short), 0, 0, 0, "Out")
        )
    );
}
